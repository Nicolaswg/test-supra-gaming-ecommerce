...
const MobileDropdown = ({ menuItem, onClose }: MobileDropdownProps) => {
  ...
  return (
    <li>
      <button
        className="flex items-center w-full gap-2 text-sm cursor-pointer capitalize  font-medium text-dark py-2 px-3 rounded-md hover:bg-primary/10 hover:text-primary transition-colors justify-between"
          ...
          {menuItem.submenu.map((sub, idx) =>
            sub.submenu ? (
              ...
              <li key={idx}>
                <Link
                  href={sub.path!}
                  className="flex items-center gap-2 text-sm font-medium text-dark py-2 px-3 rounded-md hover:bg-primary-50 hover:text-primary transition-colors"
                  onClick={onClose}
                >
                  {sub.title}
                </Link>
